...
import { createTouchControls } from "./mobile-pwa-controls.js";
import { createDungeonTouch } from "./mobile-pwa-dungeon-touch.js";

// Guard upstream layout races where minimap canvas hits 0×0 mid-relayout.
(function guardZeroDimDrawImage() {
...
// Host-side files (game.js, options.js, minimap.js) read window.DCSS_PWA to
// know when to defer to the mobile shell. Factory functions stay private to
// this module.
...
function ensureSafeAreaProbe()
{
    ...
    safeAreaProbe.id = "dcss-pwa-safe-area-probe";
...
{
    var probe = ensureSafeAreaProbe();
    var value = window.getComputedStyle(probe)
...
function usableGameHeight()
{
    ...
    var safeTop = safeAreaInset("top");
    var safeBottom = safeAreaInset("bottom");
    if (sideControlsActive())
        return Math.max(240,
            Math.floor(visualHeight() - safeTop - safeBottom));

    var usable = visualHeight() - (controlsHeight() || safeBottom) - safeTop;
...
function usableGameWidth()
{
    ...
    var sideControlsWidth = sideControlsActive() ? controlsWidth() : 0;
    var usable = visualWidth()
        - sideControlsWidth
        - safeAreaInset("left")
        - safeAreaInset("right");
    return Math.max(320, Math.floor(usable));
}
...
if (enabled)
{
    ...
    window.require(["jquery", "comm"], function ($, comm) {
        dungeonTouch = createDungeonTouch({
            $: $,
            longPressDelay: 425,
            tapMoveSlop: 14
        });
